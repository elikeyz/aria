output "ecr_repository_url" {
  ...
  value       = aws_ecr_repository.aria-api.repository_url
}

output "app_runner_service_url" {
  description = "URL of the App Runner service"
  value       = try("https://${aws_apprunner_service.aria-api.service_url}", "Not created yet - run 'terraform apply' after deploying Docker image")
}

output "app_runner_service_id" {
  description = "ID of the App Runner service"
  value       = try(aws_apprunner_service.aria-api.id, "Not created yet")
}
...
output "setup_instructions" {
  ...
  value = <<-EOT
    ...
    ✅ Aria API deployed successfully!
